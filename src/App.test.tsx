...
// detail screen in place, over the current view rather than replacing it, so the set screen's
// own dial state survives the round trip. The overlay's own element and back control are
// scoped by `.exercise-detail` throughout, since the set screen underneath is never unmounted
// and carries its own "Back" button and its own heading naming the same exercise.
...
test('L14 tapping Exercise info on the deadlift set screen opens the in-app detail for Barbell_Deadlift, and Back returns to the same set with the dials unchanged', async () => {
  const user = userEvent.setup()
  render(<App />)
  await screen.findByRole('heading', { name: 'Workout A' }, SETTLE)
  ...
  await user.click(within(overlay as HTMLElement).getByRole('button', { name: 'Back' }))
...
test('L14 tapping a row on the Exercises tab opens the same in-app detail overlay', async () => {
  ...
  const squatRowName = await screen.findByText('Barbell Squat', { selector: '.library-row-name' }, SETTLE)
  ...
  const overlay = detailOverlay()
  expect(overlay, 'no in-app detail overlay was rendered').not.toBeNull()
  expect(within(overlay as HTMLElement).getByRole('heading', { name: 'Barbell Squat' })).toBeVisible()
})
